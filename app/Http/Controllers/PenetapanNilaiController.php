...
{
    // Menampilkan data penetapan nilai berdasarkan row tertentu
    public function index(Row $row, Request $request)
    {
        $span = $request->input('span', '1 ke 2'); // default ke "1 ke 2"
    ...
    // Menyimpan data baru ke penetapan nilai
    public function store(Request $request, Row $row)
    {
        ...
        return redirect()
            ->route('row.penetapan-nilai.index', $row->id)
    ...
    public function update(Request $request, Row $row, $id)
    {
        ...
        return redirect()
            ->route('row.penetapan-nilai.index', [$row->id, 'span' => $nilai->span])
    ...
    public function destroy(Row $row, $id)
    {
        ...
        return redirect()
            ->route('row.penetapan-nilai.index', [$row->id, 'span' => $span])
