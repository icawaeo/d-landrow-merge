<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Data Sosialisasi ROW: {{ $proyek->nama_proyek }}
        </h2>
    </x-slot>
    ...
    <div class="py-2">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('row.sosialisasi.update', $sosialisasi->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
                    @csrf
                    @method('PUT')
                    <div class="space-y-4">
                        <div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                <option value="Selesai" @selected(old('status', $sosialisasi->status) == 'Selesai')>Selesai</option>
                                <option value="Belum Selesai" @selected(old('status', $sosialisasi->status) == 'Belum Selesai')>Belum Selesai</option>
                                <option value="Tertunda" @selected(old('status', $sosialisasi->status) == 'Tertunda')>Tertunda</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            <label for="lampiran_berita_acara" class="block font-medium text-sm text-gray-700">Lampiran Berita Acara (Kosongkan jika tidak ingin ganti)</label>
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">File saat ini: <a href="{{ Storage::url($sosialisasi->lampiran_berita_acara) }}" target="_blank" class="text-blue-600 hover:underline">{{ basename($sosialisasi->lampiran_berita_acara) }}</a></p>
                            </div>
                            @endif
                        </div>
                    </div>
                    <div class="flex justify-end mt-6 gap-4">
                        <a href="{{ route('row.sosialisasi.index', $proyek->id) }}" class="px-6 py-2 bg-gray-300 text-gray-700 font-semibold rounded-md hover:bg-gray-400 text-sm">
                            Batal
                        </a>
