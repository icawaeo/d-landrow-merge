<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Tambah Data Pembayaran: {{ $row->nama_proyek }}
        </h2>
    </x-slot>
    ...
    <div class="py-2">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('row.pembayaran.store', $row->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
                    @csrf
                    <div class="space-y-4">
                        <div>
                    ...
                    <div class="flex justify-end mt-6 gap-4">
                        <a href="{{ route('row.pembayaran.index', $row->id) }}" class="px-6 py-2 bg-gray-300 text-gray-700 font-semibold rounded-md hover:bg-gray-400 text-sm">
                            Batal
                        </a>
