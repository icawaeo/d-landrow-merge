        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Dokumen Pembayaran: {{ $row->nama_proyek }}
                </h2>
            </div>
            <a href="{{ route('row.pembayaran.create', $row->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700">
                + Tambah Data
            </a>
        </div>
    </x-slot>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-4">
                                            <a href="{{ route('row.pembayaran.edit', $item->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            
                                            <form action="{{ route('row.pembayaran.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                                @csrf
                                                @method('DELETE')
