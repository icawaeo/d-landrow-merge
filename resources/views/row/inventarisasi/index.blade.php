<x-app-layout>
    ...
    @push('content-header')
        <x-content-header
            ...
            :tambahDataUrl="$isReadOnly ? null : route('row-inventarisasi.create', $row->id)"
        />
    @endpush
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            @if(!$isReadOnly)
                                                <div class="flex gap-2">
                                                    <a href="{{ route('row-inventarisasi.edit', $item->id) }}" class="text-gray-600 hover:text-blue-800 p-1" title="Ubah">
                                                    ...
                                                    <form class="form-hapus" action="{{ route('row-inventarisasi.destroy', $item->id) }}" method="POST" data-nama="{{ $item->nama_kecamatan }}">
                                                        @csrf
                                                        @method('DELETE')
