<x-app-layout>
    ...
    @push('content-header')
        <x-content-header
            :proyek="$proyek"
            tahapan="Pembayaran"
            :tambahDataUrl="$isReadOnly ? null : route('pembayaran_sub.create', $proyek->id)"
        />
    @endpush
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        @if(!$isReadOnly)
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('pembayaran_sub.edit', $item->id) }}" class="text-gray-600 hover:text-blue-800 p-1" title="Ubah">
                                                ...
                                                <form class="form-hapus" action="{{ route('pembayaran_sub.destroy', $item->id) }}" method="POST" data-nama="{{ $item->nama_kecamatan }}">
                                                    @csrf
                                                    @method('DELETE')
